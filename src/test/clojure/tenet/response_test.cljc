  ...
  (testing "success response"
    (testing "destructuring as collection"
      (let [expected-type ::success
        ...
        (is (= expected-type type (:type actual) (get actual :type) (nth actual 0)))
        (is (= expected-data data (:data actual) (get actual :data) (nth actual 1)))
        (is (= :not-found (get actual ::bad-key :not-found) (get actual 2 :not-found) (nth actual 2 :not-found)))
        (is (= ::foo (-> actual (assoc :type ::foo) :type)))
        (is (= {:foo :bar} (-> actual (assoc :data {:foo :bar}) :data)))
        (is (= (assoc expected-data :foo :bar) (-> actual (assoc-in [:data :foo] :bar) :data)))
        (is (= (update expected-data :a inc) (-> actual (update-in [:data :a] inc) :data)))
        ...
        #?@(:clj
            [(is (= (.hashCode expected) (.hashCode actual)))
             ...
             (is (thrown-with-msg? IllegalArgumentException #"Response has no field for key - `42`" (assoc actual 42 :bad-value)))
            ...
            [(is (= (-hash expected) (-hash actual)))
             ...
             (is (thrown-with-msg? js/Error #"Response has no field for key - `42`" (assoc actual 42 :bad-value)))
    ...
    (testing "destructuring as map"
      (let [expected-type ::success
        ...
        (is (= expected-type type (:type actual) (get actual :type) (nth actual 0)))
        (is (= expected-data data (:data actual) (get actual :data) (nth actual 1)))
        (is (= :not-found (get actual ::bad-key :not-found) (nth actual 2 :not-found)))
        (is (= ::foo (-> actual (assoc :type ::foo) :type)))
        (is (= {:foo :bar} (-> actual (assoc :data {:foo :bar}) :data)))
        (is (= (assoc expected-data :foo :bar) (-> actual (assoc-in [:data :foo] :bar) :data)))
        (is (= (update expected-data :a inc) (-> actual (update-in [:data :a] inc) :data)))
